...
# Set groqAI API key
groq_api = "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"
...
def open_chatbot_window():
    ...
    response_frame = ttk.Frame(chatbot_window, padding=(5, 5, 5, 5))
    response_frame.pack()


    response_output = tk.Text(chatbot_window, height=13, width=100, font=("Arial", 12), bg="#FFFFFF")
    ...
    def get_response():
        ...
        # Get document paths from entries
        left_file_paths = left_entry.get().split('\n')
        right_file_paths = right_entry.get().split('\n')

        # Add contents from left documents
        for left_file in left_file_paths:
            if left_file:
                left_text = read_pdf(left_file)
                chat_history += f"First Document Content:\n{left_text}\n"

        # Add contents from right documents
        for right_file in right_file_paths:
            if right_file:
                right_text = read_pdf(right_file)
        ...
        chat_history += f"User Question: {user_input}"

        # Groq API Integration
        groq_api = "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"  # Replace with your actual key
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are a helpful assistant answering document-related queries."},
        ...
        try:
            ...
            response_output.config(state=tk.NORMAL)  # Make it editable
            ...
            response_output.config(state=tk.DISABLED)  # Disable editing again

        except Exception as e:
            response_output.config(state=tk.NORMAL)
            response_output.delete("1.0", tk.END)
            response_output.insert(tk.END, f"Exception occurred: {str(e)}")
            response_output.config(state=tk.DISABLED)

    send_button = tk.Button(input_frame, text="⚡", font=("Times New Roman", 15),
                            command=get_response, bg="#F5DEB3", bd=0, highlightthickness=0)
